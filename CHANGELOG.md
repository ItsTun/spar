...
### Experimental
- `spar coach` — **Sparky**, a *living* coach dog (companion to the quiet pane).
  A continuous animation loop (~11fps): he roams and naps when idle, perks up and
  trots over with a problem the moment Claude starts working (read from the hook
  session state), and celebrates when Claude finishes. You answer *inline* by
  typing; he grades (generous heuristic) and reacts with moods + barks, and levels
  up with your mastery (gear: headband → shades → medal → crown; learns "tricks"
  as you warm up patterns). `spar coach --demo` flips through a static preview.
  Planned: opt-in LLM grading of free-text "explain the approach" answers;
  wide-glyph width handling for pixel-perfect boxes; name + species select.
